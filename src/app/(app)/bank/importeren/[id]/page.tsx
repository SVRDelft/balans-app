...
export default async function BankimportPagina({ params }: PageProps<"/bank/importeren/[id]">) {
  ...
  return <>
    ...
    {schrijfbaar && !bestand.bevestigdOp ? <Melding toon="info" className="mb-6" titel="Controleer rekening, saldo en voorstellen">
      <p className="mb-3">{voorstellen.size === 0
        ? "De app heeft geen koppelingen gevonden waar hij zeker van is. Bevestigen neemt daarom alleen het eindsaldo over en boekt verder niets."
        : `Bevestigen neemt het eindsaldo over en verwerkt ${Math.min(50, voorstellen.size)} voorgestelde ${voorstellen.size === 1 ? "koppeling" : "koppelingen"}. De overige regels blijven klaarstaan.`}</p>
      <p className="mb-3"><strong>Koppelen kan pas na het bevestigen.</strong> Bij elke regel verschijnt dan een keuzemenu waarmee je hem aan een factuur of uitgave hangt, als nieuwe uitgave boekt, of buiten de administratie houdt. Een koppeling die niet klopt, draai je per regel weer terug. Bedragen uit een import tellen pas mee zodra je ze zo verwerkt.</p>
